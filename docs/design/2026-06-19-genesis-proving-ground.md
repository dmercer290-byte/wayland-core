# Genesis Proving Ground — Design Spec (v2, post cross-audit)
...
A standing, automated user-simulation QA system that drives the **real** Genesis Core
...
- A **Session** owns one temp `GENESIS_HOME`. A run *within* a session may relaunch the
...
- **Non-`GENESIS_HOME` globals are enumerated and individually neutralized** (the
  ...
  - **Forge discovery file** — resolves via `dirs::config_dir()` *by design*
    (`forge_discovery.rs:81`), NOT `GENESIS_HOME`. **Prerequisite task:** add a
    `GENESIS_FORGE_DISCOVERY_PATH` env override so the session can redirect it; until
