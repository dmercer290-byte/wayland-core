//! Integration tests for `genesis-core migrate hermes` (issue #228).
//!
//! Drives the public importer against a fixture Hermes home with `GENESIS_HOME`
...
//! `GENESIS_HOME` env var (same discipline as the `profile` CLI tests — avoids
...
/// A tempdir `GENESIS_HOME` (so `config.toml` resolves there) with `XDG_DATA_HOME`
/// cleared so it can't shadow the override on Linux.
fn rooted() -> (EnvGuard, TempDir) {
    let home = tempfile::tempdir().unwrap();
    let g = EnvGuard::set(&[
        ("GENESIS_HOME", Some(home.path().to_str().unwrap())),
        ("XDG_DATA_HOME", None),
    ]);
    (g, home)
}
