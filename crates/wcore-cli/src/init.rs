//! `genesis-core init` — scaffold a `.genesis/` directory in the
//! current project with a starter `config.toml` plus a `GENESIS.md`
//! at the project root.
//!
//! The `GENESIS.md` ships the portable §0-9 agent discipline (the
...
//!
//! v0.7.0 1.B.2: non-interactive by design. Reads `GENESIS_MODEL`
...
/// `GENESIS_MODEL` nor `--model` is supplied. Kept here as a constant
...
const GENESIS_MD_TEMPLATE: &str = include_str!("init_template.md");
...
pub struct InitArgs {
    ...
    /// omitted, falls back to `$GENESIS_MODEL` and then to
    /// [`PLACEHOLDER_MODEL`].
    #[arg(long)]
    pub model: Option<String>,

    /// Skip the `GENESIS.md` template at the project root. The
    /// `.genesis/config.toml` is still written.
    #[arg(long)]
...
pub struct InitOutcome {
    /// `.genesis/config.toml` was written this run.
    pub wrote_config: bool,
    /// `GENESIS.md` was written this run.
...
pub fn render_config_toml(model: &str) -> String {
    format!(
        "# Genesis-Core project config.\n\
         # v0.7.0 1.B.2: generated by `genesis-core init`. Edit freely;\n\
         ...
         # Disable in CI by setting GENESIS_MEMORY=0.\n\
         enabled = true\n\
         \n\
         [observability]\n\
...
/// Render the full `GENESIS.md`: the portable §0-9 discipline scaffold
...
pub fn render_section_10(project_name: &str, scan: &ProjectScan) -> String {
    ...
    let mut body = format!(
        "**Stack:** {} (detected by `genesis-core init`). _Refine this: describe what \
...
pub fn run(args: InitArgs) -> std::io::Result<InitOutcome> {
    ...
    let config_path = genesis_dir.join("config.toml");
    if config_path.exists() && !args.force {
        outcome.skipped.push(".genesis/config.toml".to_string());
    } else {
    ...
    if !args.skip_context {
        let context_path = root.join("GENESIS.md");
        if context_path.exists() && !args.force {
            outcome.skipped.push("GENESIS.md".to_string());
        } else {
...
pub fn print_summary(outcome: &InitOutcome) {
    if outcome.wrote_config {
        eprintln!("genesis-core: wrote .genesis/config.toml");
    }
    if outcome.wrote_context {
        eprintln!("genesis-core: wrote GENESIS.md");
    }
    for path in &outcome.skipped {
        eprintln!("genesis-core: skipped {path} (already exists; pass --force to overwrite)");
    }
    if !outcome.wrote_config && !outcome.wrote_context && outcome.skipped.is_empty() {
        eprintln!("genesis-core: nothing to do (--skip-context and config exists)");
    }
}
...
mod tests {
    ...
    fn fresh_dir_writes_both_files() {
        ...
        assert!(tmp.path().join(".genesis/config.toml").exists());
        assert!(tmp.path().join("GENESIS.md").exists());
    }
    ...
    fn force_overwrites_existing() {
        let tmp = TempDir::new().unwrap();
        std::fs::create_dir_all(tmp.path().join(".genesis")).unwrap();
        std::fs::write(tmp.path().join(".genesis/config.toml"), "old").unwrap();
        std::fs::write(tmp.path().join("GENESIS.md"), "old").unwrap();
    ...
    #[test]
    fn deployed_genesis_md_ships_the_discipline_scaffold() {
        let tmp = TempDir::new().unwrap();
        run(args_in(tmp.path())).unwrap();
        let md = std::fs::read_to_string(tmp.path().join("GENESIS.md")).unwrap();
        ...
        assert!(
            ...
            "unsubstituted placeholder leaked into GENESIS.md"
        );
    }
